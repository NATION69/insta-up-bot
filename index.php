...
{
    sleep(30);
    $api = file_get_contents('https://kiwibro.000webhostapp.com/firaswapper.php/?uid='.$idd.'&submit=submit');
...
class EzTG
{
    ...
    private function is_telegram()
    {
        ...
        { //preferisco non usare x-forwarded-for xk si può spoof
        ...
        { //gram'''s ip : https://core.telegram.org/bots/webhooks
            return true;
        }
        else
        {
    ...
    public function newKeyboard($type = 'keyboard', $rkm = array(
...
class EzTGKeyboard
{
    public function __construct($type = 'keyboard', $rkm = array(
        'resize_keyboard' => true,
        'keyboard' => array()
    ))
    {
    ...
    public function add($text, $callback_data = null, $type = 'auto')
    {
        if ($this->type === 'inline')
        {
            ...
            if (!isset($this->keyboard['inline_keyboard'][$this
                ->line]))
            {
        ...
        {
            if (!isset($this->keyboard['keyboard'][$this
                ->line]))
            {
